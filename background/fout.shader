...
uniform sampler2D _texVignette : register(s1);
...
;
;
float3 _Gamma(in float3 _value, in float _param)
{
{
return vec3(pow(abs(_value.x), _param), pow(abs(_value.y), _param), pow(abs(_value.z), _param));
;
}
}
float3 _GammaLod0(in float3 _value, in float _param)
{
{
return vec3(pow(abs(_value.x), _param), pow(abs(_value.y), _param), pow(abs(_value.z), _param));
;
...
;
float4 _vignetteColor(in float4 _inColor, in sampler2D _texMultiply)
{
{
float4 _blend = gl_texture2D(_texMultiply, _v_vTexcoord);
float4 _outColor = vec4((_blend.xyz * _inColor.xyz), _inColor.w);
return lerp(_outColor, _inColor, 0.5);
;
}
}
float4 _vignetteColorLod0(in float4 _inColor, in sampler2D _texMultiply)
{
{
float4 _blend = gl_texture2DLod0(_texMultiply, _v_vTexcoord);
float4 _outColor = vec4((_blend.xyz * _inColor.xyz), _inColor.w);
return lerp(_outColor, _inColor, 0.5);
;
}
...
(_outColor = _vignetteColor(_outColor, _texVignette));
