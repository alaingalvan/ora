...
if (menu_type == 0)
{
    ...
    for (var a = 0; a < ds_list_size(list); a++)
    {
        ...
        if (aa > 0.0)
        {
            ...
            draw_set_alpha(aa);
            draw_text_color((xx + button_w - 4) * scale, (button_y + button_h - 48) * scale, string_hash_to_newline(ds_list_find_value(list, a)), c_white, c_white, c_white, c_white, 0.75 * aa);
            draw_set_alpha(1);
        }
    }
...
if (menu_type == 1)
{
    ...
    draw_text_color(x1 * scale, y1 * scale, string_hash_to_newline(cur_menu), c_white, c_white, c_white, c_white, trans_alpha);
    draw_set_font(fon_main);
    ...
    for (var i = 0; i < size; i += 1)
    {
        ...
        if (data != "")
        {
            ...
            draw_text_color((1200 - 8) * scale, (y1 + (i * 64)) * scale, string_hash_to_newline(data), c_white, c_white, c_white, c_white, trans_alpha);
            draw_text_color((1152 - 32) * scale, (y1 + (i * 64)) * scale, string_hash_to_newline("<"), c_white, c_white, c_white, c_white, trans_alpha);
            draw_text_color((1280 - 16) * scale, (y1 + (i * 64)) * scale, string_hash_to_newline(">"), c_white, c_white, c_white, c_white, trans_alpha);    
            draw_set_halign(fa_left)      
        }
        if (i == cur_select)
        {
            ...
            draw_text_ext_color((x1 - 4) * scale, (736) * scale, string_hash_to_newline(mainmenu_description(txt)), 32 * scale, 342 * scale, c_white, c_white, c_white, c_white, trans_alpha);
        }
        draw_set_font(fon_main);
        draw_text_color((x1) * scale, (y1 + (i * 64)) * scale, string_hash_to_newline(txt), c_white, c_white, c_white, c_white, trans_alpha);
