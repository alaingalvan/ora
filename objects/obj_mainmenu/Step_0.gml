...
if (cur_line > ds_list_size(list) - list_w) {
  list_scroll_band =
    -(ds_list_size(list) - list_w) * (button_w + button_spacing);
}
...
if (menu_type == 0) {
  ...
  for (var a = 0; a < ds_list_size(list); a++) {
    ...
    inregion =
      mouse_x > xx * s &&
      mouse_x < (xx + button_w) * s &&
      mouse_y > (button_y - button_y_add[a]) * s &&
      mouse_y < (button_y + button_h) * s;
...
if (mouse_check_button(mb_left)) {
  ...
  var _inregion;
  _inregion =
    mouse_y > (button_y - button_y_max) * s &&
    mouse_y < (button_y + button_h) * s &&
    mouse_x > xmin * s &&
    mouse_x < xmax * s;
  ...
  if (_inregion) {
    if (
      abs(mouse_y - mb_left_event_y) > 2.0 ||
      abs(mouse_x - mb_left_event_x) > 2.0
    ) {
      mouse_dragging = true;
    }
  }
  ...
  if (global.gamepad_connected[0]) {
    ...
    if (moving) {
      ...
      if (menu_type == 1) {
        ...
        if (angle == 180) {
          mainmenu_data_previous(
            ds_list_find_value(list, cur_select + cur_line)
          );
        }
      }
    }
