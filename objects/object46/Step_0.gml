...
//Movement Mechanics
if (left == true) {
  hspd = max(-maxspd, hspd - accspd);
}
if ((right = true)) {
  hspd = min(maxspd, hspd + accspd);
}
...
for (var __i = 0; __i < ceil(abs(vspd)); __i++) {
  ...
  if (place_meeting(x, y + sign(vspd), block) == 1 && up == true) {
    vspd *= -1.005;
...
for (
  var __i = 0;
  __i < (Speed / 100) * global.blurvar;
  __i++ //Repeat as many times as there is space between the current and previous position
) {
  with (instance_create(xx, yy, obj_foe_versa_blur)) {
    foeid = other.id;
...
if (instance_exists(obj_orb)) {
  if (collision_point(x, y, obj_orb, 1, 0) && obj_orb.Speed > 30) {
    hp = 0;
    hspd *= -1;
    vspd *= -1;
    obj_orb.hspd *= -1;
