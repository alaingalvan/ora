function camera_update() {
    ...
    if (!global.cutscene) {
        if (instance_exists(obj_orb)) {
            //Follow Ora
            var viewport_width = camera_get_view_width(0);
            var viewport_height = camera_get_view_height(0);
            x = obj_orb.x + 128;
            y = obj_orb.y + 128;
            z = obj_orb.z;
            ...
            if (rotating && global.debug) {
                angle_x_band = prev_angle_x + ((mouse_x - global.mb_middle_event_x) * global.mouse_sensitivity_x)
                if (prev_angle_y >= -90 && prev_angle_y <= 90) {
                    angle_y_band = prev_angle_y + ((mouse_y - global.mb_middle_event_y) * global.mouse_sensitivity_y)
                }
            }
            ...
            if (controller)
	        {
                ...
                looking = abs(rightstick_x) > global.gamepad_deadzone[0] || abs(rightstick_y) > global.gamepad_deadzone[0];

                if (looking) {
                    angle_x_band += ((rightstick_x - global.gamepad_deadzone[0]) / (1 - global.gamepad_deadzone[0])) * (global.gamepad_sensitivity_y[0]);
                    angle_y_band += ((rightstick_y - global.gamepad_deadzone[0]) / (1 - global.gamepad_deadzone[0])) * (global.gamepad_sensitivity_y[0]);
                }
            }
