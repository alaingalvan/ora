...
for (var a = 0; a < ds_list_size(list); a++) {
  ...
  if (string_lower(ds_list_find_value(list, a)) == "vsync") {
    if (a < ds_list_size(list) - 1) //change that 1 to however many arguments you have.
    {
      ...
      if (string_lower(ds_list_find_value(list, a + 1)) == "on") {
        global.vsync = true;
        display_reset(global.antialias, global.vsync);
  ...
  if (string_lower(ds_list_find_value(list, a)) == "antialiasing") {
    if (a < ds_list_size(list) - 1) //change that 1 to however many arguments you have.
    {
      if (string_isdigit(ds_list_find_value(list, a + 1))) {
        if (real(tokenlist[a + 1]) % 2 == 0 && real(tokenlist[a + 1]) >= 0 && real(tokenlist[a + 1]) <= 8) {
          string_history += "Set Anti-Aliasing to  " + tokenlist[a + 1] + ".#";
          global.antialias = real(tokenlist[a + 1]);
          display_reset(global.antialias, global.vsync);
        } else {
          string_history += "ERROR: Did not give valid parameter. [0, 2, 4, 8]#";
        }
      } else {
