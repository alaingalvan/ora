function d3d_model_load_ext(
  ...
  //Syntax: d3d_model_load_ext(ind,fname,flipnormals,fliptexcoords,scale)

  //the model (ind) must already be created like in d3d_model_load()
  //scale is because the d3d_transform_add_scaling(xy,ys,zs) destroys the normals
  //"scale", "flipnormals" and "fliptexcoords" don't affect *.mod files
  /////////////////////////////////////
  //created debug variables:
  //modelload[0]=vetexes loaded
    ...
    do {
      ...
      do {
        ...
        if (string_count("// end of .Vertex", row) == 0) {
          ...
          for (var __i = 0; __i < (3 - 1 + 3 + (2 - 1)); __i++)
          {
        ...
        if (string_count("// end if .Index", row) == 0) {
          ...
          for (var __i = 0; __i < 3; __i++) {
            ...
            d3d_model_vertex_normal_texture(
              argument0,
              ds_list_find_value(data, i + 0) * scale,
              ds_list_find_value(data, i + 1) * scale,
              ds_list_find_value(data, i + 2) * scale,
              flipnormals * ds_list_find_value(data, i + 3),
              flipnormals * ds_list_find_value(data, i + 4),
              flipnormals * ds_list_find_value(data, i + 5),
              ds_list_find_value(data, i + 6),
              tex_y
            );
            modelload[0] += 1;
            t -= 1;
      ...
      do {
        ...
        string_count(".Vertex", row) == 1 ||
        string_count("// End of file", row) == 1
      );
    ...
    do {
      if ((string_count("v ", row) = 0)) {
        do {
          ...
          string_char_at(row, 1) == "v" &&
          string_char_at(row, 2) == " "
        );
      }
      ...
      do {
        ...
        (string_char_at(row, 1) == "v" && string_char_at(row, 2) == "n") ||
        (string_char_at(row, 1) == "v" && string_char_at(row, 2) == "t") ||
        (string_char_at(row, 1) == "f" && string_char_at(row, 2) == " ")
      );
      ...
      do {
        ...
        if ((string_char_at(row, string_length(row)) = " "))
          row = string_copy(row, 1, string_length(row) - 1);
        ...
        if (edges <= 3) {
          for (t = 0; t < edges; t += 1) {
            ...
            d3d_model_vertex_normal_texture(
              argument0,
              ds_list_find_value(v_x, faces[t, 0]) * scale,
              ds_list_find_value(v_y, faces[t, 0]) * scale,
              ds_list_find_value(v_z, faces[t, 0]) * scale,
              flipnormals * ds_list_find_value(n_x, faces[t, 2]),
              flipnormals * ds_list_find_value(n_y, faces[t, 2]),
              flipnormals * ds_list_find_value(n_z, faces[t, 2]),
              ds_list_find_value(u, faces[t, 1]),
              tex_y
            );
            modelload[0] += 1;
          }
          ...
          for (t = 2; t < edges; t += 1) {
            ...
            d3d_model_vertex_normal_texture(
              argument0,
              ds_list_find_value(v_x, faces[0, 0]) * scale,
              ds_list_find_value(v_y, faces[0, 0]) * scale,
              ds_list_find_value(v_z, faces[0, 0]) * scale,
              flipnormals * ds_list_find_value(n_x, faces[0, 2]),
              flipnormals * ds_list_find_value(n_y, faces[0, 2]),
              flipnormals * ds_list_find_value(n_z, faces[0, 2]),
              ds_list_find_value(u, faces[0, 1]),
              tex_y
            );
            modelload[0] += 1;
            ...
            d3d_model_vertex_normal_texture(
              argument0,
              ds_list_find_value(v_x, faces[t - 1, 0]) * scale,
              ds_list_find_value(v_y, faces[t - 1, 0]) * scale,
              ds_list_find_value(v_z, faces[t - 1, 0]) * scale,
              flipnormals * ds_list_find_value(n_x, faces[t - 1, 2]),
              flipnormals * ds_list_find_value(n_y, faces[t - 1, 2]),
              flipnormals * ds_list_find_value(n_z, faces[t - 1, 2]),
              ds_list_find_value(u, faces[t - 1, 1]),
              tex_y
            );
            modelload[0] += 1;
            tex_y = ds_list_find_value(v, faces[t, 1]);
        ...
        if (string_count("f ", row) == 0) {
          do {
            ...
            (string_char_at(row, 1) == "f" && string_char_at(row, 2) == " ") ||
            (string_char_at(row, 1) == "v" && string_char_at(row, 2) == " ") ||
            file_text_eof(file)
          );
        }
      ...
      d3d_model_primitive_end(argument0);
      while (!(string_count("v ", row) = 1 || file_text_eof(file))) {
        row = file_text_read_string(file);
        file_text_readln(file);
      }
  ...
  if (string_count(".x", argument1) > 0) {
    ...
    do {
      ...
      row = file_text_read_string(file);
      file_text_readln(file); //don't read numb of vertexes
      do {
        row = file_text_read_string(file);
        file_text_readln(file);
      ...
      t = 0;
      do {
        row2 = file_text_read_string(file);
        file_text_readln(file);
      ...
      row = file_text_read_string(file);
      file_text_readln(file); //don't read numb of normals
      ...
      row = file_text_read_string(file);
      file_text_readln(file); //don't read numb of MeshTextureCoords
      ...
      for (i = 0; i < numb_faces; i += 1) {
        ...
        for (t = 1; t <= faces[i, 0]; t += 1) {
          ...
          d3d_model_vertex_normal_texture(
            argument0,
            ds_list_find_value(v_x, faces[i, t]) * scale,
            ds_list_find_value(v_y, faces[i, t]) * scale,
            ds_list_find_value(v_z, faces[i, t]) * scale,
            flipnormals * ds_list_find_value(n_x, faces[i, t]),
            flipnormals * ds_list_find_value(n_y, faces[i, t]),
            flipnormals * ds_list_find_value(n_z, faces[i, t]),
            ds_list_find_value(u, faces[i, t]),
            tex_y
          );
          modelload[0] += 1;
        }
      ...
      do {
        ...
        if ((string_count("};", row) = 0)) {
          ...
          for (var __i = 0; __i < 3; __i++)
          {
            ...
            d3d_model_vertex_normal_texture(
              argument0,
              ds_list_find_value(v_x, faces[i]) * scale,
              ds_list_find_value(v_y, faces[i]) * scale,
              ds_list_find_value(v_z, faces[i]) * scale,
              flipnormals * ds_list_find_value(n_x, faces[i]),
              flipnormals * ds_list_find_value(n_y, faces[i]),
              flipnormals * ds_list_find_value(n_z, faces[i]),
              ds_list_find_value(u, faces[i]),
              tex_y
            );
            modelload[0] += 1;
            i += 1;
    ...
    do {
      ...
      do {
        ...
        if (string_count("Face", row2) == 1) {
          ...
          d3d_model_vertex(
            argument0,
            ds_list_find_value(v_x, t) * scale,
            ds_list_find_value(v_y, t) * scale,
            ds_list_find_value(v_z, t) * scale
          );
          modelload[0] += 1;
          str = string_copy(row, 1, string_pos(":", row) - 1);
          row = string_delete(row, 1, string_pos(":", row));
        ...
        (string_count("Face", row2) == 0 &&
          string_count("Smoothing", row2) == 0) ||
        file_text_eof(file)
      );

      while (!(string_count("Vertex list:", row) == 1 || file_text_eof(file))) {
        row = file_text_read_string(file);
        file_text_readln(file);
      }
      ds_list_clear(v_x);
