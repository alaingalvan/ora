function mainmenu_data_previous(argument0) {
  //mainmenu_data_next(category)
  //This script returns a string based on the inputted string. This is for options related stuff.
  var sel;
  ...
  if (!trigger) {
    if (string_lower(cur_menu) == "gameplay") {
      ...
      if (sel == "subtitles") {
        global.captions = (global.captions + 1) % 3;
      }
      if (sel == "commentary") {
        global.commentary = !global.commentary;
      }
      if (sel == "language") {
        global.language = (global.language + 1) % 1;
      }
    }

    if (string_lower(cur_menu) == "graphics") {
      if (sel == "resolution") {
        res_preset_set((res_preset_get() + 1) % 5);
      }
      if (sel == "aspect ratio") {
        aspect_preset_set((aspect_preset_get() + 1) % 3);
      }
      if (sel == "fullscreen") {
    ...
    if (string_lower(cur_menu) == "advanced graphics") {
      if (sel == "anti-aliasing") {
        global.antialiasing = (global.antialiasing + 2) % 10;
      }
      if (sel == "framelimit") {
        global.framelimit = (global.framelimit + 10) % 130;
